...
test_face = cv2.imread("face_test.png") #face_test.png  lena1.jpg
...
while video_capture.isOpened(): #True:
    ...
    for (x, y, w, h) in faces:
        ...
        for (ex, ey, ew, eh) in eyepairs:
            ...
            #cv2.imshow('img_glasses_mask bg', img_glasses_mask)
            #cv2.imshow('img  fg', img)

            #cv2.imshow('mask_inv bg', mask_inv)
            #cv2.imshow('mask fg', mask)
